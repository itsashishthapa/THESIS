...
class SteamEnv(gym.Env):
   ...
   17) t_k = Delta_t * k
    ...
    def __init__(
        ...
        T_s_min=120.0, # minimum storage temperature in Celsius
        T_s_max=260.0, # maximum storage temperature in Celsius
    ...
    def observation_index_map(self):
        """Returns an index map for the observation vector."""
        mapping = {
            0: "T_s_k_norm",
            1: "g_grid_k_norm",
            2: "P_WT_k_norm",
    ...
    def action_index_map(self):
        """Returns an index map for the action vector."""
        return {
            0: "beta1_k",
            1: "beta2_k",
            2: "R_k",
    ...
    def _HTHP_surrogate(self, T_I_k, m_I_k, T_III_k, R_k):
        """HTHP surrogate (equations F1, F2, F3)."""
        T_I = float(T_I_k)
        m_I = float(m_I_k)
        T_III = float(T_III_k)
        R = float(np.clip(R_k, 0.8, 1.53))
        
        # T_II_k (hot side outlet)
        T_II_k = (
    ...
    def step(self, action):
        ...
        # F4/F5/F10: bypass logic and mutually exclusive mode handling (stricter)
        charge_active = (beta1_k < 1.0 - 1e-6)
        discharge_active = (beta2_k < 1.0 - 1e-6)
        mode_violation_k = float(charge_active and discharge_active)

        if charge_active and not discharge_active:
            mode_k = 1
            beta2_k = 1.0
        elif discharge_active and not charge_active:
            mode_k = 2
            beta1_k = 1.0
        ...
        if mode_k == 1:
            ...
            # F9 then F7 (discharging branch)
            T_4_k = T_SG_out - self.epsilon_dch * (T_SG_out - T_0_k)
            Q_s_dch_k = (T_4_k - T_SG_out) * m_I_k * 3 * self.c_p_f * (1.0 - beta2_k)
            Q_s_ch_k = 0.0
            T_1_k = T_0_k
            T_2_k = T_0_k
        else:
